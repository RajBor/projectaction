 ...
 *
 * Loads a company by ticker, assembles multi-year financials via a
 * unified pipeline (Screener first, RapidAPI fallback, snapshot as
 * last resort), runs DCF + comparables + book-value methods, pulls
 * peer statistics, and renders a consulting-grade report that prints
 * to PDF cleanly.
 *
 * ── Source parity (signed-in ≡ signed-out ≡ public) ─────────────
 * Screener's BSE/NSE-filed Ind-AS financials are the canonical source
 * for EVERY viewer, regardless of login state. That's deliberate — an
 * audited P&L is an audited P&L; two users looking at the same firm
 * must see identical numbers, otherwise the report loses credibility.
 *
 * RapidAPI is kept as a fallback for tickers Screener doesn't cover
 * (fresh IPOs, delisted names) and is only requested for authenticated
 * users so its paid quota isn't spent on public anonymous traffic.
 *
 ...
 *   • rely on Screener multi-year + Company snapshot (same source chain
 *     signed-in users use — the only difference is the RapidAPI
 *     fallback never fires for them, which in practice means the same
 *     Screener numbers either way for any ticker on Screener),
...
function ReportBody({
  ...
  // ── Screener multi-year — canonical source for every viewer ──
  //
  // Fetched from /api/data/screener-financials/[ticker] which is
  // entirely public (no auth) and caches the scraped HTML parse in
  // user_companies.financials_multi for 30 days. The result is the
  // PRIMARY source feeding buildFinancialHistory: signed-in, signed-off
  // and public anonymous visitors all render from the same numbers.
  // Previously RapidAPI was primary for auth users which meant the
  // same company showed different revenue / EBITDA depending on login
  // state — an inconsistency an analyst can't defend.
  const [screenerYears, setScreenerYears] = useState<import('@/lib/live/screener-fetch').ScreenerYearData[] | null>(null)
  useEffect(() => {
    let cancelled = false
    // Always fetch — every viewer gets the same data. The DB cache
    // absorbs repeat calls so there's no server-side cost after the
    // first visit per ticker per month.
  ...
  return (
    <section className="dn-page dn-page-cover dn-cover">
      ...
      <div className="bottom">
        ...
        {history.source === 'screener' ? 'Screener.in — BSE/NSE Ind-AS filings'
          : history.source === 'rapidapi' ? 'NSE/BSE Annual Reports (Yahoo)'
          : 'Company snapshot (single-year estimate)'}
        <br />
    ...
    <section className="dn-page">
      ...
      <div className="dn-narrative" style={{ marginTop: 12 }}>
        ...
        <p className="dn-mutedtxt" style={{ fontSize: 9 }}>
          ...
          {history.source === 'screener' ? 'Screener (Ind-AS filings)'
            : history.source === 'rapidapi' ? 'RapidAPI (Yahoo)'
            : 'Company snapshot'} (
